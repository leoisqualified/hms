import React from "react";
import { assets } from "../assets/assets/assets";

const Header = () => {
  return (
    <div className="flex flex-col md:flex-row flex-wrap bg-primary">
      {/* Left Side */}
      <div className="md:w-1/2 flex flex-col items-start justify-center gap-4 py-10 px-6 md:py-[10vw] md:mb-[30px]">
        <p className="text-3xl md:text-4xl lg:text-5xl text-white font-semibold leading-tight md:leading-tight lg:leading-tight">
          Book Appointment <br />
          With Trusted Doctors
        </p>
        <div className="flex flex-col md:flex-row items-center gap-3 text-white text-sm font-light">
          <img src={assets.group_profiles} alt="" className="w-28" />
          <p>
            Simply browse through our extensive list of trusted doctors,{" "}
            <br className="hidden sm:block" />
            Schedule an appointment
          </p>
        </div>
        <a
          href="#speciality"
          className="flex items-center gap-2 bg-white px-8 py-3 rounded-full text-gray-600 text-sm hover:scale-105 transition-all duration-300"
        >
          Book Appointment{" "}
          <img src={assets.arrow_icon} alt="" className="w-3" />
        </a>
      </div>

      {/* Right Side */}
      <div className="md:w-1/2 relative flex items-center justify-center md:justify-end">
        <img
          src={assets.header_img}
          alt=""
          className="w-full md:w-[90%] h-auto md:absolute md:bottom-0 rounded-lg"
        />
      </div>
    </div>
  );
};

export default Header;
